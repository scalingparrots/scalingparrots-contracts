...
publish() {
  ...
  log "Publishing @scalingparrots/contracts on npm"
  cd contracts
  npm publish --tag "$dist_tag" --otp "$(prompt_otp)"
  cd ..

  if [[ "$dist_tag" == "latest" ]]; then
    npm dist-tag rm --otp "$(prompt_otp)" @scalingparrots/contracts next
  fi
}
...
push_and_publish() {
  ...
  log "Pushing release branch and tags to master"
  push_release_branch_and_tag
